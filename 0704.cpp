...
void division(HugeInt dividend, HugeInt divisor, HugeInt& quotient, HugeInt& remainder)
{   
    quotient.size = dividend.size - divisor.size+1;
    quotient.digit = new int[quotient.size]();
        ...
        for (int i = quotient.size-1; i >= 0; i--) {   //每一位的商
            ...
            while (less(newdivisor, dividend)) {       //除數小於被除數就做
...
void subtraction(HugeInt& minuend, HugeInt subtrahend)
{
    ...
    {
        minuend.digit[j] = minuend.digit[j] - subtrahend.digit[j];
        j++;
    }
    while (j < minuend.size)
    {
        minuend.digit[j] = minuend.digit[j];
        j++;
    }
    for (i = 0; i < minuend.size - 1; i++)
    {
    ...
    while (minuend.size > 1 && minuend.digit[minuend.size - 1] == 0)
        minuend.size--;
}
...
bool less(HugeInt hugeInt1, HugeInt hugeInt2)
{
    ...
    for (int i = hugeInt1.size - 1; i >= 0; i--) {
        if (hugeInt1.digit[i] < hugeInt2.digit[i])return true;
